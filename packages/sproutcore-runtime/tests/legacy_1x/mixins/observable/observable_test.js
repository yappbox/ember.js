...
var get = SC.get, set = SC.set, fmt = SC.String.fmt, w = SC.String.w;
...
test("getting values should call function return value", function() {
  ...
  var keys = w('computed computedCached dependent dependentCached');

  keys.forEach(function(key) {
    equals(object.get(key), key, fmt('Try #1: object.get(%@) should run function', key));
    equals(object.get(key), key, fmt('Try #2: object.get(%@) should run function', key));
  });

  // verify each call count.  cached should only be called once
  w('computedCalls dependentCalls').forEach(function(key) {
    equals(object[key].length, 2, fmt('non-cached property %@ should be called 2x', key));
  });

  w('computedCachedCalls dependentCachedCalls').forEach(function(key) {
    equals(object[key].length, 1, fmt('non-cached property %@ should be called 1x', key));
  });
...
test("setting values should call function return value", function() {
  ...
  // get each property twice. Verify return.
  var keys = w('computed dependent computedCached dependentCached');
  var values = w('value1 value2');

  keys.forEach(function(key) {

    equals(object.set(key, values[0]), object, fmt('Try #1: object.set(%@, %@) should run function', key, values[0]));

    equals(object.set(key, values[1]), object, fmt('Try #2: object.set(%@, %@) should run function', key, values[1]));
    
    equals(object.set(key, values[1]), object, fmt('Try #3: object.set(%@, %@) should not run function since it is setting same value as before', key, values[1]));
  ...
  keys.forEach(function(key) {
    ...
    expectedLength = 3;
    equals(calls.length, expectedLength, fmt('set(%@) should be called the right amount of times', key));
    for(idx=0;idx<2;idx++) {
      equals(calls[idx], values[idx], fmt('call #%@ to set(%@) should have passed value %@', idx+1, key, values[idx]));
    }
  });
